<template>
  <div class="comic-guochanmanhua-list">
...
export default {
  ...
  methods: {
    getAllList () {
      axios.get('http://localhost:31875/static/static-json/comicGuoChanManHua.json').then(this.getAllListSucc)
    },
    getAllListSucc (res) {
      const data = res.data
      const result = data.result
      if (result) {
        console.log('this is in comics GuoChanManHua' + result)
        this.allList = data.comicGuoChanManHuaList
      }
    }
  },
  mounted () {
